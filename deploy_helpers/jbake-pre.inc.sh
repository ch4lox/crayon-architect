...
if [ -z "${DEPLOY_BUILD_OUTPUT_PATH}" ]; then
	export DEPLOY_BUILD_OUTPUT_PATH="./output"
fi
echo "DEPLOY_BUILD_OUTPUT_PATH is $DEPLOY_BUILD_OUTPUT_PATH"

if [ ! -d "${DEPLOY_BUILD_OUTPUT_PATH}" ]; then
	export CRAYON_PRE_DEPLOY_SKIP="true";
	export CRAYON_CMD_DEPLOY_SKIP="true";
	
	echo "${DEPLOY_BUILD_OUTPUT_PATH} does not exist!";
fi

if [ "$CRAYON_PRE_DEPLOY_SKIP" != "true" ]; then
	if [ ! -f "${DEPLOY_BUILD_OUTPUT_PATH}/index.html" ]; then
		export CRAYON_PRE_DEPLOY_SKIP="true";
		export CRAYON_CMD_DEPLOY_SKIP="true";
	
		echo "${DEPLOY_BUILD_OUTPUT_PATH}/index.html does not exist!";
	fi
fi
